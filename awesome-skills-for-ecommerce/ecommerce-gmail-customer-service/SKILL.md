...
3. Read `system-prompt.md`, `workflow.md`, `persona.md`, `user_memory.md` and `config.json` in the running directory. If the running directory does not exist or new files are missing, run `python3 scripts/configure.py init` first; initialization must not overwrite existing running files.
...
4. Pull the current activities and applicable shipping, cancellation, refund, return, exchange, warranty, subscription and privacy policies; long policies first form a summary of terms with source, version, region, validity period and exceptions.
